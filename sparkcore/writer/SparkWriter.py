...
class SparkWriter:
    ...
    def does_database_exist(self, database: str):
        db_lst = self.spark_session.catalog.listDatabases()
        return True if len([db for db in db_lst if db.name == database]) > 0 else False
    ...
    def does_table_exist(self, database: str, checked_table: str) -> bool:
        ...
        return True if len([table for table in table_lst if table.name == checked_table]) > 0 else False
